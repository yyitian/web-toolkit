...
>
> **2026-06-24 处置批次**：逐项讨论后，本轮完成 2/3/4(部分)/5/6/7；**P0-1（测试）研讨后决定不做**（见该项）；P1-4 的 `<Teleport>`、P1-5 的 SuperButton 子路径拆分明确不做（理由见各项）。
>
> **2026-06-24 发布准备**：版本退回 `0.1.0`、补 MIT `LICENSE`、`package.json` 加 `sideEffects`、完成首次 git 提交（分支 `main`）。剩余发布动作（建 GitHub 远程仓库、设 `NODE_AUTH_TOKEN`、`pnpm publish`）待手动执行。

---

## P0 — 影响发布质量 / 静默失败

### ⊘ 1. 补齐测试框架与核心组件测试（研讨后决定不做）

> **决策（2026-06-24）**：研讨后决定跳过测试流程，本项不再列入计划。`backlog.md` 第 3 项的同类待办亦随此决策搁置。CLAUDE.md「仓库没有配置测试框架」表述保持不变。
